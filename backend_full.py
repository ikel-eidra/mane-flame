...
"""
Mane - The Flame 🔥
Full-featured Gemini-powered autonomous AI agent
With Notion MCP and Google Drive integration
"""
...
# Import Notion MCP connector
from mane_notion_mcp import NotionMCP
...
# Mane's enhanced identity with Notion/Drive integration
...
✅ **Read/write to Notion EIDRA Hub**
✅ **Store memories and logs in Notion**
✅ **Access Google Drive for file storage**
...
8. Save documentation to Notion
9. Report complete results with URLs
...
- notion_save_memory: Save memory to Notion
- gdrive_upload: Upload to Google Drive
...
7. Save documentation to Notion
8. Provide working URLs
...
class ManeAgent:
    def __init__(self):
        ...
        try:
            ...
            print(f"⚠️  Notion MCP not available: {e}")
    ...
    def notion_search(self, query: str) -> Dict[str, Any]:
        """Search Notion"""
        if not self.notion_enabled:
            return {"success": False, "error": "Notion MCP not available"}
        return self.notion.search_pages(query)
    
    def notion_create_page(self, title: str, content: str, parent_id: str = None) -> Dict[str, Any]:
        """Create Notion page"""
        if not self.notion_enabled:
            return {"success": False, "error": "Notion MCP not available"}
        return self.notion.create_page(parent_id or "workspace", title, content)
    
    def notion_save_memory(self, title: str, content: str, tags: List[str] = None) -> Dict[str, Any]:
        """Save memory to Notion"""
        if not self.notion_enabled:
            return {"success": False, "error": "Notion MCP not available"}
        return self.notion.write_memory_entry(title, content, tags)
...
async def status():
    ...
    return JSONResponse({
        ...
        "capabilities": [
            ...
            "Notion Integration", "Google Drive"
        ],
        "integrations": {
            "notion": mane.notion_enabled,
...
async def notion_search(q: str):
    """Search Notion"""
    result = mane.notion_search(q)
    return JSONResponse(result)

if __name__ == "__main__":
    print("🔥 Starting Full-Featured Mane - The Flame...")
    ...
    print("Notion MCP:", "✅ Enabled" if mane.notion_enabled else "⚠️  Not available")
